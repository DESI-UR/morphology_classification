#!/usr/bin/env python3
"""
Generate differently sized Legacy Surveys cutouts,
...
LAYER           = "ls-dr10-resid"   # LS data release
...
def viewer_cutout_url(ra, dec, size_pix, pixscale=PIX_SCALE, bands=BANDS, layer=LAYER, sga=SGA):
    """Build LS viewer cutout URL."""
    params = dict(ra=ra, dec=dec, layer=layer, size=size_pix, pixscale=pixscale, bands=bands, sga=sga)
    return f"{VIEWER_URL}?{urlencode(params)}"

def fetch_cutout(ra, dec, size_pix, pixscale, outpath, session=SESSION, timeout=TIMEOUT,
    ...
    url = viewer_cutout_url(ra, dec, size_pix, pixscale)
...
def safe_stem(targetid, ra, dec, size_pix, bands=BANDS):
...
def main(targets_csv, outdir, bands=BANDS, layer=LAYER, pix_scale=PIX_SCALE, size=CUTOUT_SIZE_PIX,
    ...
    logging.info("Loaded %d targets from %s", len(df), targets_csv)
    ...
    for _, row in df.iterrows():
        ...
        size_pix = int(row["bin_id"] * size)

        stem = safe_stem(tid, ra, dec, size_pix, bands=bands)
        ...
        if not os.path.exists(fits_out):
            t0 = time.time()
            ok = fetch_cutout(ra, dec, size_pix, pix_scale, fits_out,
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Generate 152x152 LS cutouts + PNG previews")
    ap.add_argument("--csv", required=True, help="Path to TARGETS_SSL.csv (lowercase columns)")
